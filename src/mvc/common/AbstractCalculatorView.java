...
public abstract class AbstractCalculatorView extends JFrame {
    ...
    protected ActionListener calcListener;
    ...
    public void addCalculateListener(java.awt.event.ActionListener l) {
        this.calcListener = l;
    }

    protected JLabel lbl(String t) {
        JLabel l = new JLabel(t);
        l.setForeground(UIStyle.TEXT_SECOND);
        l.setFont(UIStyle.FONT_LABEL);
        return l;
    }

    protected void styleCombo(JComboBox<String> cb) {
        cb.setBackground(UIStyle.BG_FIELD);
        cb.setForeground(UIStyle.TEXT_PRIMARY);
        cb.setFont(UIStyle.FONT_BTN_SM);
        cb.setBorder(BorderFactory.createLineBorder(UIStyle.SEPARATOR, 1, true));
    }

    protected void styleRadio(JRadioButton r, Color c) {
        r.setBackground(UIStyle.BG_DARK);
        r.setForeground(c);
        r.setFont(UIStyle.FONT_LABEL);
